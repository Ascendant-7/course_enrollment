CREATE TABLE
  users (
    ...
    CONSTRAINT chk_users_username_length CHECK (char_length(username) BETWEEN 6 AND 14),
    -- CONSTRIANT chk_users_username_format CHECK (username REGEXP '^[A-Za-z0-9._-]{6,14}$'),
    -- -- RFC 5322
    -- CONSTRAINT chk_users_email_format CHECK (
    --   email REGEXP '^[A-Za-z0-9]+([._%+-][A-Za-z0-9]+)*@[A-Za-z0-9]+([.-][A-Za-z0-9]+)*\.[A-Za-z]{2,}$'
    -- ) CONSTRAINT chk_users_firstname_format CHECK (first_name REGEXP "^[A-Za-z'-]+$"),
    -- CONSTRAINT chk_users_lastname_format CHECK (first_name REGEXP "^[A-Za-z'-]+$"),
